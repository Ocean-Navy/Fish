...
Paste the generated app env block into the private web-host env file. The faucet private key belongs to the web app because `/api/testnet/faucet` signs claims; it should not be copied into `.env.ocean-demo-stack`. When `--contract-deployment` is present, the generator reads the deployed test OCEAN/Test USDC addresses from the ignored contract deployment artifact. Deployment-derived faucet addresses, contract write actions, and settlement submission require a Base Sepolia artifact (`chainId=84532`).
