...
- V0 may ship all public URL targets as sections on `/`, but code should be split so `/ask`, `/dashboard`, `/roadmap`, `/providers`, `/credits`, `/docs`, and `/api` can become real routes without rewriting content.
